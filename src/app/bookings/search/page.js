...
export default function ShuttleSearchPage() {
		...
		async function fetchData() {
			try {
				...
				const shuttlesRes = await fetch(
					`/api/bookings/search?fromStop=${fromStop}&toStop=${toStop}&shift=${shift}&day=${day}`
				);
				if (shuttlesRes.ok) {
	...
	const formatDayType = (day) => {
		return day === 'weekday' ? 'Class Day (Mon-Fri)' : 'Weekend (Sat-Sun)';
	};
		...
		<div className='container mx-auto px-4 py-8 max-w-4xl'>
			...
			<Card className='border-slate-200 shadow-sm mb-6'>
				<CardHeader>
					...
					<CardDescription>
						Shuttle search results based on your criteria
					</CardDescription>
				</CardHeader>
				<CardContent>
						...
						<div className='flex items-center gap-2'>
							...
							<div>
								...
								<div className='font-medium'>
									{fromStopDetails?.stopName || fromStop}
								</div>
							</div>
						</div>
					...
					<CardContent className='p-8'>
						<div className='text-center'>
							...
							<p className='text-slate-500'>
								There are no shuttles available for your selected route and
								time. Please try different locations or times.
							</p>
						</div>
					</CardContent>
					<CardFooter>
					...
					{shuttles.map((shuttle) => (
						<Card
							...
							<CardHeader className='pb-2'>
								...
								<CardDescription>
									{fromStopDetails?.stopName || fromStop} to{' '}
									{toStopDetails?.stopName || toStop}
								</CardDescription>
							</CardHeader>
								...
								<div className='mt-4 flex items-center gap-2'>
									<div className='w-full bg-slate-100 rounded-full h-2'>
										<div
											...
											style={{
												width: `${
													((shuttle.capacity - shuttle.seats) /
														shuttle.capacity) *
													100
												}%`
											}}></div>
							...
							<CardFooter>
								<Button
									...
									onClick={() =>
										router.push(
											`/bookings/confirm?shuttleId=${shuttle.shuttleId}&fromStop=${fromStop}&toStop=${toStop}&fare=${shuttle.fare}`
										)
									}>
									Book Now
								</Button>
